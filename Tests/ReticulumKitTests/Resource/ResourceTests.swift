...
struct ResourceAdvertisementTests {
    ...
    @Test("is_response flag and request_id round-trip in ADV")
    func responseResourceFlags() throws {
        ...
        let adv = ResourceAdvertisement(
            ...
            totalSegments: 1,
            requestId: requestId,
            flags: 0x11, // encrypted + is_response
            hashmap: Data(repeating: 0x03, count: 8)
        )
        let packed = try adv.pack()
        let unpacked = try ResourceAdvertisement.unpack(packed)
        #expect(unpacked.isResponse)
        #expect(unpacked.requestId == requestId)
        #expect(unpacked.flags == 0x11)
    }
}
